...
?>
		...
		?>
				  ...
				  <div class='row' >
					...
					<?PHP
					$searchcategory=''; //default value for search.
					$searchprice='200';
					...
					$searchprice=substr($searchprice,0,50); //limiting the string length to 50 characters
					$searchcategory=substr($searchcategory,0,50);
					
					$productcondition=substr($productcondition,0,50);
					
					$processedprice="<=$searchprice"; //setting the logic on price
					
						}
						
					if($searchprice=='greaterThan') //This sets in when price is greater than 50000
						{
							$processedprice=">=50000";
						}
					...
					$SQL = "SELECT * FROM products WHERE productid $combined AND category='$searchcategory' AND offerprice $processedprice AND status !='removed' AND productcondition='$productcondition' ORDER BY productid DESC ";
					$result1 = mysqli_query($conn, $SQL);
						...
						$num_rows1 = mysqli_num_rows($result1);
									...
									if ($num_rows1 < 1)
												{
												echo "<p style='color: red; text-align:center;'>No product met your search criteria.</p>";
												die;	
												}	
						else
						{
