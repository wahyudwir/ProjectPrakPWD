...
?>
		...
		?>
				  ...
				  <div class='row' >
					...
					<h4 style='text-align:center;'>Latest Arrivals (NEW PRODUCTS)</h4>
					...
					$SQL = "SELECT * FROM products WHERE productid $combined AND status !='removed' AND productcondition='new' ORDER BY productid DESC ";
				...
				if($counter>6) //Two rows, each of 3 columns will have been filled by this loop count value.
				{
					...
					echo "<img src='$imagelink'   alt='image' width='100%'  /> <br />";
		...
		//End of JavaScript
			...
			<?PHP //START of BLOCK 2  ?>
				  <div class='row' >
					...
					<h4 style='text-align:center;'>Featured NEW Products </h4>
					...
					$SQL = "SELECT * FROM products WHERE productid $combined AND status !='removed' AND productcondition='featured' ORDER BY productid DESC ";
					$result1 = mysqli_query($conn, $SQL);
						if(!$result1)//checking whether the database connection has been successful
							{
								echo 'Database error';
		...
		echo "<p id='$trend'style='text-align:right'></p> 
			...
			<?PHP //START of BLOCK 3  ?>
				  <div class='row' >
					...
					<h4 style='text-align:center;'>Second-hand Products </h4>
					...
					$SQL = "SELECT * FROM products WHERE productid $combined AND status !='removed' AND productcondition='second_hand' ORDER BY productid DESC ";
					$result1 = mysqli_query($conn, $SQL);
						if(!$result1)//checking whether the database connection has been successful
							{
								echo 'Database error';
