...
?>	 
		...
		if(isset($_POST['unfulfilled']))
					...
					$status=''; //giving blank value as it was before the order was marked fulfilled. 
		...
		echo "<h4 style='background-color: silver; color:green; padding: 10px;' >Order was successfully marked as unfulfilled   </h4>";
			}
		...
		echo "<h4 style='background-color: silver; color:green; padding: 10px; ' >Order was successfully removed from fulfilled orders   </h4>";
			}
					...
					if($check==0)
						{
						echo" <h4 style='text-align:center;'>Fulfilled Orders </h4>";
							
						$SQL = "SELECT * FROM orders2 WHERE orderstatus ='fulfilled' ORDER BY orderid ASC ";
						...
						echo "<tr> <td> $no. </td> <td style='padding:5px'> $orderid </td> <td style='padding:5px'> $ordertotal </td> <td style='padding:5px'> $grandtotal </td>  <td style='padding:5px'> <a href='?view=1&orderid=$orderid' >View </a> | <a href='lap_mbl.php' > Cetak </a></td></tr>";
					...
					if($check==1)
						...
						$SQL = "SELECT * FROM orders2 WHERE orderstatus ='fulfilled' AND orderid='$orderid' ORDER BY orderid ASC ";
						$result1 = mysqli_query($conn, $SQL);
							if(!$result1)//checking whether the database connection has been successful
								{
									echo 'Database error';
					...
					<input  style='padding:10px; color: green; ' type = 'Submit' name = 'unfulfilled' value = 'Mark order as unfulfilled' /> 
					</form>						</strong></p>
						
						<p style='padding:10px; '> <strong style='background-color:silver; '> 
						<form name='form5' method='POST' action='' >
					<input type='hidden'  name='orderid' value='$orderid' required  />
					<input style='padding:10px; color: green; ' type = 'Submit' name = 'removed' value = 'Remove order from fulfilled orders' /> 
					</form>
						</strong></p>
						";
