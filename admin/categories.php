...
?>	 
				...
				<div class="row" style='background-color:silver;' >
				  ...
				  <div class='row' >
					...
					$SQL = "SELECT * FROM categories WHERE categoryid $combined AND status !='removed' ORDER BY categoryid DESC ";
				...
				if($counter>6) //Two rows, each of 3 columns will have been filled by this loop count value.
				{
					...
					$id=$row['categoryid']; //fetching the last Category id.
					$categoryid=$row['categoryid'];
					$category=$row['category'];
					
					$form1="form$categoryid";
					$results1="results$categoryid";
					$redirect="redirect$categoryid";
					$trend="trend$categoryid";
					$loading="loading$categoryid";
					$addbutton="add$categoryid";
					
					echo " ";
					
					//Form to submit details asyncronously.
				echo"	
					<form id= '$form1' style='background-color:silver;' >
					<h4> $category   <br /> </h4>
        <input type='hidden' name='categoryid' value='$categoryid' >
		
		<input type='submit' name='add' id='$addbutton' value='Delete category'>
		...
		echo"
				...
				//End of hide  'Add to Cart' button
					 ...
					 $('#'+form1).submit(function(event){
						...
						$.post('../includes/delete_category_process.php', formValues, function(data){
		...
		//End of JavaScript
			...
			?>
				...
				<form id= 'formcategory' style='background-color:silver;' >
					<h4>  Create New Category   <br /> </h4>
        <label>Category Name  </label> <input type='text' name='newcategory' required  id='addtext' />
		
		<input type='submit'  id='addcategory' value='Create category' />
		
			<div style='padding: 0px; background-color: black; color: white; position: fixed; top: 30%; left: 20%; z-index:400000; width: 60%; text-align:center' id='resultscategory'> </div>		
					</form>
		...
		<script type='text/javascript'>
				...
				$(document).ready(function(){
						...
						var form1 = 'formcategory';
						var results1='resultscategory';
						var addbutton='addcategory';
						var addtext='addtext';
						
			
				
				//Beginning of hide  'Add button' button
				$('#'+addbutton).click(function() 
				{
				$('#'+addbutton).hide();
				});
				...
				//End of hide  'Add to Cart' button
					 ...
					 $('#'+form1).submit(function(event){
						...
						// Send the form data using post
						$.post('../includes/create_category_process.php', formValues, function(data){
							// Display the returned data in browser
							$('#'+results1).html(data);
						});
					});
